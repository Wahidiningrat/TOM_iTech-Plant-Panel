...
function getTrustedDomains(): Set<string> {
  ...
  // Add REPLIT_DOMAINS (always available in Replit)
  ...
  if (process.env.PUBLIC_BASE_URL) {
    try {
      ...
      console.error('Invalid PUBLIC_BASE_URL:', e);
    }
  }
...
export async function setupAuth(app: Express) {
  ...
  // Helper function to ensure strategy exists for a trusted domain
  const ensureStrategy = (domain: string) => {
    // Validate domain is trusted
    if (!isTrustedHost(domain)) {
      throw new Error(`Untrusted domain: ${domain}`);
    }
  ...
  app.get("/api/login", (req, res, next) => {
    ...
    if (!isTrustedHost(req.hostname)) {
      return res.status(403).json({ 
        message: "Forbidden: Untrusted host",
  ...
  app.get("/api/callback", (req, res, next) => {
    // Validate hostname before processing callback
    if (!isTrustedHost(req.hostname)) {
      return res.status(403).json({ 
        message: "Forbidden: Untrusted host",
        hostname: req.hostname 
  ...
  app.get("/api/logout", (req, res) => {
    // Validate hostname before logout
    if (!isTrustedHost(req.hostname)) {
      return res.status(403).json({ 
        message: "Forbidden: Untrusted host",
        hostname: req.hostname 
